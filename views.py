...
def AppointmentAction(request):
    if request.method == 'POST':
        ...
        dbcursor = dbconnection.cursor()
        dbcursor.execute(
    "INSERT INTO appointment (appointment_id, patient_name, doctor_name, disease_details, prescription, appointment_date, booking_date) VALUES (%s, %s, %s, %s, %s, %s, %s)",
    [bid, username, doctor, disease, 'Pending', date, today]
)
...
def Appointment(request):
    ...
    doctor = request.GET.get('doctor', '')   # get doctor from URL
...
def getImage(name):
    ...
    with mysqlConnect:
        result = mysqlConnect.cursor()
        result.execute("select aadhar_img, aadhar_no from patients where ='"+name+"'")
        lists = result.fetchall()
        for ls in lists:           
            name = ls[0]
            aadhar_no = ls[1]
